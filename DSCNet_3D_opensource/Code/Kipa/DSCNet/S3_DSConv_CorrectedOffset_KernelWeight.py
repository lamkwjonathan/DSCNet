...
class DCN_Conv(nn.Module):
    def __init__(self, in_ch, out_ch, kernel_size, extend_scope, morph, if_offset, device):
        ...
        self.weights_conv = nn.Conv1d(in_ch, in_ch, kernel_size=self.kernel_size, stride=1, padding=0)
        self.weights_bn = nn.BatchNorm1d(self.kernel_size)
    ...
    def forward(self, f):
        ...
        deformed_feature = deformed_feature.reshape(input_shape[0]*input_shape[2]*input_shape[3]*input_shape[4], input_shape[1], self.kernel_size)
        deformed_feature_padded = torch.nn.functional.pad(deformed_feature, (self.kernel_size//2, self.kernel_size//2), mode='circular').to(self.device)
        weights = self.weights_conv(deformed_feature_padded)
        weights = weights.permute(0, 2, 1)
        weights = self.weights_bn(weights)
        weights = self.relu(weights)
        weights = weights.reshape(self.kernel_size, input_shape[0], input_shape[1], input_shape[2], input_shape[3], input_shape[4])
        weights = torch.softmax(weights, dim=0)
        weights_new = weights.detach().clone()
        deformed_feature = deformed_feature.reshape(self.kernel_size, input_shape[0], input_shape[1], input_shape[2], input_shape[3], input_shape[4])
