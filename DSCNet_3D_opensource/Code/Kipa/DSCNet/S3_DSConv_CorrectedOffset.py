...
class DCN(object):
    ...
    def _bilinear_interpolate_3D(self, input_feature, z, y, x):
        z = z.reshape([-1]).float() # [N*D*W*H*K]
        y = y.reshape([-1]).float() # [N*D*W*H*K]
        x = x.reshape([-1]).float() # [N*D*W*H*K]
        ...
        x1 = torch.clamp(x1, zero, max_x)  # [N*D*W*H*K]
        ...
        base = torch.matmul(base, repeat)  # [N, 1] * [1, N*D*W*H]  ==> [N, K*D*W*H]
