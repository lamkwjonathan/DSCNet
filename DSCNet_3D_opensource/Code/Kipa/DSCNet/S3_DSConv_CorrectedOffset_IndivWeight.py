...
class DCN_Conv(nn.Module):
    ...
    def forward(self, f):
        ...
        weights_conv = nn.Conv1d(input_shape[2]*input_shape[3]*input_shape[4], input_shape[2]*input_shape[3]*input_shape[4], kernel_size=self.kernel_size, stride=1, padding=0)
        weights_bn = nn.BatchNorm1d(self.kernel_size)
        ...
        deformed_feature_padded = torch.nn.functional.pad(deformed_feature, (self.kernel_size//2, self.kernel_size//2), mode='circular')
        weights = weights_conv(deformed_feature_padded)
        weights = weights.permute(0, 2, 1)
        weights = weights_bn(weights)
