...
class DCN(object):
    ...
    def _coordinate_map_3D(self, offset, if_offset):
        ...
        if if_offset: # x_new are positions, offsetX are vectors in (-1, 1)
            ...
            x_new[center + 1] = x_new[center] + offset3[center + 1]
            z_new[center - 1] = z_new[center] + offset1[center - 1]
            y_new[center - 1] = y_new[center] + offset2[center - 1]
            x_new[center - 1] = x_new[center] + offset3[center - 1]
            ...
            for index in range(2, center + 1):
                offset_cat_pos = torch.stack([offset1[center + index], offset2[center + index], offset3[center + index]], dim=0)
                z_offset_pos, y_offset_pos, x_offset_pos = self._offset_interpolate_3D(offset_cat_pos, z_new[center + index - 1], y_new[center + index - 1], x_new[center + index - 1])
                ...
                offset_cat_neg = torch.stack([offset1[center - index], offset2[center - index], offset3[center - index]], dim=0)
                z_offset_neg, y_offset_neg, x_offset_neg = self._offset_interpolate_3D(offset_cat_neg, z_new[center - index + 1], y_new[center - index + 1], x_new[center - index + 1])
